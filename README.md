...
1. Clone & Configure
Bash
git clone https://github.com/Nyakeruma-Hezron/eventflow.git
cd eventflow
cp .env.example .env
...
Client interacts with the Google Sign-In widget.

Google authenticates the user and returns a secure ID Token directly to the React frontend.

React securely transmits this token to the backend (/api/auth/google/).

Django validates the cryptographic signature of the token against Google's public keys.

Upon verification, Django creates (or retrieves) the user and issues standard Access and Refresh JWTs.

The frontend stores these tokens to authorize subsequent API requests.
